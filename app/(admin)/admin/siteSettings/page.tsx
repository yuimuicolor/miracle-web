...
export default function AdminSiteSettingsPage() {
  ...
  return (
    <div className="mx-auto">
      <AdminHeader
        title="사이트 기본 설정"
        subtitle="브랜드 정보 및 하단 푸터 정보를 관리합니다."
      ...
      <div className="space-y-8 bg-white p-8 rounded-3xl shadow-sm border border-slate-100">
        ...
        <section>
          ...
          <div className="flex items-center gap-6">
            ...
            <button
              ...
              className="px-4 py-2 bg-slate-800 text-white rounded-lg text-admin- hover:bg-slate-700 transition-colors"
            >
              로고 교체
            </button>
            <input
              ...
              onChange={(e) => {
                ...
                if (file) handleReplaceImage(file, "logo");
              }}
            />
          </div>
        </section>
        ...
        <section className="flex flex-col lg:grid lg:grid-cols-2 gap-6">
          {/* 메인 배경화면 설정 */}
          <div>
            <h3 className="text-admin-body font-bold mb-4">
              메인 배경화면 설정
            </h3>
            <div className="flex items-center gap-6">
              <div className="relative w-40 h-20 bg-slate-300 rounded-xl overflow-hidden border border-slate-200">
                <Image
                  src={
                    items?.mainBgPreviewUrl || items?.mainBackgroundSrc || ""
                  }
                  alt="p"
                  fill
                  className="object-cover"
              ...
              <input
                ...
                onChange={(e) => {
                  const file = e.target.files?.[0];
                  if (file) handleReplaceImage(file, "mainBackground");
                }}
              />
            </div>
          </div>
          ...
          <div>
            ...
            <div className="flex items-center gap-6">
              ...
              <input
                ...
                onChange={(e) => {
                  const file = e.target.files?.[0];
                  if (file) handleReplaceImage(file, "galleryBackground");
                }}
              />
            </div>
          </div>
        ...
        <section className="flex flex-col lg:grid lg:grid-cols-2 gap-6">
          <div>
            <label className="block text-admin- font-semibold mb-2">
              브랜드명
            </label>
            <AdminInput
          ...
          <div>
            ...
            <AdminInput
              value={items?.businessRegistrationNumber || ""}
              onChange={(v) => handleChange("businessRegistrationNumber", v)}
            />
          </div>
          ...
          <div>
            ...
            <AdminInput
              value={items?.businessHours || ""}
              onChange={(v) => handleChange("businessHours", v)}
              placeholder="Information 란에 노출될 영업시간을 입력해 주세요."
            />
          </div>
          ...
          <div>
            ...
            <AdminInput
              value={items?.mapTitle || ""}
              onChange={(v) => handleChange("mapTitle", v)}
              placeholder="Google 지도에 표시될 이름을 입력해 주세요."
            />
          </div>
          <div>
            <label className="block text-admin- font-semibold mb-2">
            ...
            <AdminInput
              value={items?.mapLink || ""}
              onChange={(v) => handleChange("mapLink", v)}
              placeholder="https://www.google.com/maps/place/로 시작하는 정확한 주소를 입력해 주세요."
            />
          </div>
        </section>
        ...
        {/* SNS 설정 섹션 */}
        <section>
          <h3 className="text-admin-body font-bold mb-4 text-slate-700">
            SNS 연결
          </h3>
          <div className="grid gap-4">
            {Object.entries(items?.snsConfig || {}).map(
              ([platform, config]) => (
                <div
                  key={platform}
                ...
                >
                  ...
                  <AdminInput
                    value={config.href}
                    placeholder="URL을 입력하세요 (https://...)"
                    onChange={(v) => handleSNSChange(platform, "href", v)}
                  />
                </div>
              ),
            )}
