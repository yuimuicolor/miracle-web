      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        {[
          ...
          <div key={i} className="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
            <p className="text-admin-subtitle text-gray-500 font-medium">{stat.label}</p>
            <p className={`text-admin-body font-bold mt-1 ${stat.color}`}>{stat.value}</p>
          </div>
        ))}
      </div>

      <div className="bg-white p-8 rounded-xl shadow-sm border border-gray-200">
        <h3 className="text-admin-subtitle font-bold mb-4">공지사항 및 팁</h3>
